...
function update() {
	...
	_.each(_.omit(config, 'nbob', 'project', 'envConfigMap', 'addListener'), function(commandConfig, commandName) {
		/*jshint maxdepth:10*/
...
commands.addListener = function(listener) {
	listeners.push(listener);
};

update();

require('./config').addListener(function() {
	update();
