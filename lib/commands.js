...
var cmdBuild = new Command('build').addSubCommands([
	new Command('html').addSubCommands([
		new Command('l10n', 'Localize files', 'l10n'),
		new Command('minify', 'Minify HTML'),
	...
	new Command('js').addSubCommands([
		...
		new Command('amd', 'Optimize EM AMD modules', 'amd')
	]),
...
var cmdMake = new Command('make', 'Analyze, build and test').addDependencies([
	cmdAnalyze,
	cmdBuild,
	cmdTest
]);
...
var commands = [
	...
	new Command('deploy', 'Copy a make to S3', 'deploy-s3').addDependencies([ cmdMake ])
];
