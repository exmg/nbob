...
function getName(command, list, parts) {
	...
	_.each(lengthenMap, function(name, abbrev) {
		if (!shortenMap[name]) {
			shortenMap[name] = abbrev;
		}
	});

	for (var i = 0; i < list.length; i++) {
		var sub = list[i];
		var subName = getName(command, sub.subCommands, parts.concat(shortenMap[sub.name]));
		if (subName) {
			return subName;
		}
	}
...
commands.getName = function(command) {
	return getName(command, commands, []);
};
