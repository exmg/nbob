...
function processBatch(pName, pConfig, pLog, inputs, outputs) {
	var inputSize = 0;
	...
	outputs.forEach(function(output) {
		...
		output.resolve = function(data) {
			...
			if (output.trackRatio) {
				pLog.ok('%s (%s %s)', output.path, bytes(outputSize), percentage(outputSize, inputSize));
			} else {
				pLog.ok('%s', output.path);
			}
		};
	});

	return scoop.send(pName, pConfig, pLog.prefix, inputs, outputs);
}
...
module.exports = function(execCommands) {
	...
	var filesTasks = _.filter(commands.select(execCommands), 'processor').map(function(command) {
		...
		var processor = getProcessor(pName, pConfig, shortName);
				...
				var batchPromises = batches.map(function(batch, batchIndex) {
					...
					return Promise.all(inputPromises).then(function(inputs) {
						...
						return fs2.lstat(cacheDir).then(function() {
							...
							wasCached = false;
							return processBatch(pName, pConfig, pLog, inputs, outputs).then(function() {
