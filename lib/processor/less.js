...
processor.getBatches = function(inputPaths, inputFiles) {
	var log = this.log;
	var projectDir = this.config.project.dir;
	var importMap = {};
	var imported = [];
	...
	// TODO: Recurse imports, e.g: add imports of imports .. to importMap records
	...
	parser.parse(text, function(error, tree) {
		if (error) {
			...
			try {
				...
				output.reject(new Error(util.format('%s [%s:%d:%d]', e.message, e.filename, e.line, e.index)));
			}
		}
	});
};
