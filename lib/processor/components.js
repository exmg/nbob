...
//
// OR: "make:bower":
//   does bower install with dependencies defined in nbob config in /build/bower/
//   for each bower component
...
// PS: Preferably do this in a way with nbob caching based on bower.json for maximum speed with incremental builds
//
// This should then consume those files as inputs (e.g: through FSResolver)
// This should also facilitate copying webcomponents-lite.min.js (e.g: to /lib/)
...
processor.getBatches = function(inputPaths) {
	...
	var targetIndex = inputPaths.indexOf(target);
	if (targetIndex < 0) {
		return [];
	}

	var inputs = [ { path: target, trackRatio: true } ];
	inputPaths.forEach(function(inputPath, index) {
		if (index !== targetIndex) {
			inputs.push({ path: inputPath, isReadOnly: true, trackRatio: true });
		}
	});

	return [ new Batch(inputs, [ target ]) ];
};
...
processor.process = function(inputs, outputs) {
	...
	var vulcanizeOptions = _.extend({
		inputUrl: input.path,
		fsResolver: new FSResolver(config, inputs)
	}, config.vulcanize);
