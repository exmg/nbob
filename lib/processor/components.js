...
processor.getBatches = function(inputPaths) {
	var bowerPath = inputPaths[0] === this.config.bower && inputPaths.shift();
	if (!inputPaths.length) {
		return [];
	}

	var inputs = inputPaths.map(function(inputPath) {
		// We need to leave component source files in place for image and font etc. references
		// and for any included demo pages to remain working etc. etc.
		return { path: inputPath, isReadOnly: true };
	});
	...
	return [ new Batch(inputs, [ { path: this.config.output, trackRatio: true } ]) ];
};
