...
processor.process = function(inputs, outputs) {
	...
	if (mapInput) {
		...
		// Validate sourcemap as postcss validates sourcemap.mappings. Postcss
		// throws on empty mappings.
		isEmptySourcemap = !mapInputData.mappings;
	}

	return processor.process(input.data, {
		from: input.path,
		to: output.path,
		map: isEmptySourcemap ? false : {
			inline: false,
			prev: mapInputData,
			sourcesContent: true,
			annotation: false
		...
		mapOutput.resolve(isEmptySourcemap ? originalMapInputData : result.map.toString());
