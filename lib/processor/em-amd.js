...
function getDependencyErrors(modules) {
	return modules.reduce(function(depErrors, module) {
		...
		if (deps.length > 0 && params.length === 0 && module.body.indexOf('.importModules(this.dependencies)') >= 0) {
			// E.g: var c = playtotv.importModules(this.dependencies);
			// Too difficult to check and deprecated for a long time so you're on your own; no checking here
