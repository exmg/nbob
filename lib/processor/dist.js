...
processor.getBatches = function(inputPaths) {
	...
	if (oldBuildDir && newBuildDir && (oldBuildDir !== newBuildDir)) {
		...
		try {
			...
			// Sometimes, I don't understand why, Windows throws: EPERM, operation not permitted
			// Not renaming is inefficient, but not fatal, so at least for now, I will just catch and warn
			log.warn('Build dir rename failed: %s', error.message);
		}
	}

	// Remove files that are no longer in inputPaths
	_.difference(oldPaths, newPaths).forEach(function(removedPath) {
		log.ok('Removed: %s', removedPath);
