...
var nbobConfig = require('../config').get().nbob;
...
module.exports = function(config, log, inputFiles) {
	...
	log.info('Listening on port: %d (press ctrl-c, esc or q key to exit)', port);

	if(nbobConfig.openbrowser) {
		opener('http://'+nbobConfig.host+':'+port);
	}
