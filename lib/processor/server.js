...
function mwDefault(config) {
	return function(req, res, next) {
		// Skip if request is not for HTML
		var accept = req.headers && req.headers.accept || '';
		if (req.method !== 'GET' || !/text\/html/.test(accept)) {
			return next();
		}

		// If requested file does not exist and would not result in a directory listing then change to default URL
		var server = config.options.server;
		var reqPath = server.baseDir + url.parse(req.url).pathname;
		fs.stat(reqPath, function(err, stats) {
			if (err || !(stats.isFile() || stats.isDirectory() && server.directory)) {
