...
function substituteMD5s(log, files, file) {
	return file.getText().then(function(text) {
		var newText = text.replace(/([^'"]+)(\?[^'"]*)__MD5__/g, function(match, path, between) {
			// Oh crap, referenced file.getMD5() will be async as well of course
			// I need an async Promise based string replace approach
			var md5 = 'TODO';
			return path + between + md5;
		});
...
module.exports = function(config, log, files) {
	...
	return digestFiles(files).then(function(digest) {
		...
		// Only substitute contents of text files (and in specified order)
		var textFiles = files.filter(function(file) {
			return minimatches(file.path, config.textFiles);
		});

		// First perform basic contents substition
		return Promise.all(textFiles.map(function(file) {
			return substituteContents(substitutes, log, files, file);
		})).then(function() {

			// Then substitute MD5 references
			return Promise.all(textFiles.map(function(file) {
				return substituteMD5s(log, files, file);
			})).then(function() {

				// And finally return complete collection of files again
				return files;
			});
		});
	});
