...
processor.process = function(inputs, outputs) {
	...
	// Convert all whitespace to a single space and trim it
	text = text.replace(/\s+/g, ' ').trim();

	// Remove comments (multi-line thanks to above)
	text = text.replace(/<!--(.*?)-->/g, function(match, paren1) {
		// Leave Internet Explorer conditional comments
		return /\[if IE/.test(paren1) ? match : '';
	});

	// Replace attribute double quotes (") by single quotes (') to reduce JSON escape overhead
