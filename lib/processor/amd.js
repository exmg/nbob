...
function parse(text) {
	...
	text.split('\n').forEach(function(line) {
		...
		} else if (line.indexOf('sourceMappingURL') !== -1) {
			sourceMapLine = line;
		} else {
...
processor.getBatches = function(inputPaths) {
	return inputPaths.map(function(inputPath) {
		// TODO: Accept source map input and adapt it while pruning etc.
		// OR: Do this before source maps are involved?
...
processor.process = function(inputs, outputs) {
	...
	outputs[1].resolve(graph.graphviz());
	...
	output.resolve(parsed.otherText + '\n' + _.pluck(orderedModules, 'text').join('\n') + '\n' + parsed.sourceMapLine);
};
