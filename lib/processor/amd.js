...
function parse(text) {
	...
	text.split('\n').forEach(function(line) {
		if (moduleLines) {
			...
			if (line.indexOf('});') === 0) {
				var text = moduleLines.join('\n');
				moduleLines = null;

				var matches = text.match(
					/^(em|playtotv)\.define\('([\w\.]+)', \[([\w\s\.',]*)\], function\(([\w\s,]*)\) \{([\w\W]*)\}\);/
				);
				var id = matches[2];
				var deps = matches[3].trim();
				deps = deps.length > 0 ? deps.substr(1, deps.length - 2).split(/',\s*'/) : [];
				var params = matches[4].trim();
				params = params.length > 0 ? params.split(/,\s*/): [];
				modules.push({
					id: id,
					deps: deps,
					params: params,
			...
			if (/^(em|playtotv)\.define\(/.test(line)) {
				moduleLines = [ line ];
			} else {
...
processor.process = function(inputs, outputs) {
	...
	if (undefDeps.length > 0) {
		return output.reject(util.format('%s: Undefined dependencies: %s', inputPath, undefDeps.map(function(rec) {
	...
	modules = pruneUnusedModules(exports, modules);
	...
	if (graph.nodes.length > 0) {
		return output.reject(util.format('%s: Circular dependencies, see: %s', inputPath, graph.chart()));
	}
