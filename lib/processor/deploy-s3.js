...
function awsPromisifyListFn(fn, thisArg, key) {
	...
	return function list(params) {
		return promisified.call(thisArg, params).then(function(data) {
			var items = data[key];
			if (data.IsTruncated && data.NextMarker === undefined) {
				throw new Error('AWS Data is truncated but lacking NextMarker for next page');
			}
...
module.exports = function(config, log, inputFiles) {
	...
	return Promise.apply([
		...
		s3.listObjects({ Delimiter: ',' }) // Note: Delimiter is required for NextMarker (paging)
		...
		log.info('Finished at: ' + new Date().toString().substr(0, 24));
