...
function init() {
	// Removed :names from template to prevent wrapping in narrow (width 80) consoles
	bar = new ProgressBar('[:bar] :percent :etas', { total: 0, incomplete: ' ' });
	counterMap = {};
	totalMap = {};
	active = false;
}
...
function clear() {
	if (active) {
		active = false;
		var stream = bar.stream;
		if (stream && stream.clearLine && stream.cursorTo) {
			stream.clearLine();
			stream.cursorTo(0);
		}
		bar.lastDraw = null;
