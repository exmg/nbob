	...
	if (processor instanceof Processor) {
		...
		batches.forEach(function(batch, i) {
			...
			schedule.addParallel(function() {
				...
				var inputFiles = files.getList(batch.inputPaths);
				var promise = processor.process(inputFiles, batch.outputPaths).then(function(outputFiles) {
					if (isReadOnly) {
						assert.strictEqual(outputFiles, undefined, 'Read only processor should return no output');
					} else {
						var outputPaths = _.pluck(outputFiles, 'path').sort();
						...
						files.process(inputFiles, outputFiles);
					}

					bar.finish(shortName);
					pLog.spam('Finished batch #%d after: %s', i + 1, timeSince(processorStart));
					onFinishBatch();
				}, function(error) {
					pLog.info('Rejected batch #%d after: %s', i + 1, timeSince(processorStart));
					onFinishBatch();
